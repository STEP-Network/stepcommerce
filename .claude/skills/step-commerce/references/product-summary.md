...
## Repo-status
Repoet bygges i Claude-chat og round-trippes som zip (`step-commerce-repo-vX.zip`). Struktur: `packages/widget` (runtime + playground m. mocks), `packages/db/schema.sql`, `apps/admin` (Next.js-stub), `docs/` (spec + prototyper), `CLAUDE.md` (status-tjekliste — opdatér den hver session). Færdigt: trin 1 + widget-runtime m. begge templates. Næste: trin 2–3 (feed-fetcher + product rules). Hosting-beslutning: Vercel + Neon, kobles på i trin 9.

...
.fp-page.flipped{transform:rotateY(-102deg);opacity:0;transition:transform 1.1s cubic-bezier(.55,.06,.24,.99),opacity .3s .8s}
...
.fp-splash{position:absolute;top:58px;right:8px;width:62px;height:62px;background:var(--sc-cta-bg);color:var(--sc-price);border-radius:50%;display:flex;flex-direction:column;align-items:center;justify-content:center;font-weight:800;transform:rotate(8deg);box-shadow:0 2px 5px rgba(0,0,0,.25)}
.fp-splash .p{font-size:1em;line-height:1}
...
function offer(p: ServeProduct): HTMLElement {
  ...
  el.append(p.imageUrl ? h('img', { src: p.imageUrl, alt: p.title, loading: 'lazy' }) : h('div', { class: 'ph' }));
...
export function renderForum(
  ...
  if (meta.advertiserLogoUrl) avatar.append(h('img', { src: meta.advertiserLogoUrl, alt: meta.advertiserName }));
  else avatar.append(h('span', { style: 'color:#fff;font-weight:700;font-size:.8em' }, [meta.advertiserName.slice(0, 2).toUpperCase()]));
  const who = h('div', { class: 'fp-who' });
  const nameLine = h('div');
  nameLine.append(
    h('span', { class: 'fp-name' }, [meta.advertiserName]),
  ...
  const ctaUrl = wrapClick(meta.ctaUrl ?? products[0]?.clickUrl ?? '#');
  ...
  if (meta.advertiserLogoUrl) front.append(h('img', { src: meta.advertiserLogoUrl, alt: meta.advertiserName }));
  else front.append(h('div', { class: 'name' }, [meta.advertiserName]));
  ...
  if (badgeProduct?.badge) {
    ...
    splash.append(h('div', { class: 'p' }, [badgeProduct.badge]));
    front.append(splash);
  }
